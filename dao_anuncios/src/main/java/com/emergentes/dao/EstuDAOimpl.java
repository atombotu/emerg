...
public class EstuDAOimpl extends ConexionDB implements EstuDAO {
    ...
    public void insert(estu estudiante) throws Exception {
        try {
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("INSERT into estudiantes (nombre, apellidos, seminarios, confirmado, fecha_inscripcion) values (?, ?, ?, ?, ?)");
    ...
    public void update(estu estudiante) throws Exception {
        try {
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("UPDATE estudiantes set nombre = ?, apellidos = ?, seminarios = ?, confirmado = ?, fecha_inscripcion = ? where id = ?");
            ps.setString(1, estudiante.getNombre());
            ps.setString(2, estudiante.getApellidos());
            ps.setInt(3, estudiante.getSseminarios());
            ps.setBoolean(4, estudiante.isConfirmado());
    ...
    public estu getById(int id) throws Exception {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                est.setSseminarios(rs.getInt("seminarios"));
